...
export default class VueBigDataTableClass extends Vue {
    ...
    @Prop({type: Boolean, default: false}) selectable?: boolean; // 是否可选择单元格
    @Prop({type: Boolean, default: false}) paste?: boolean; // 是否可粘贴，如果设为true，则selectable效果为true
    @Prop({type: Boolean, default: false}) sortable?: boolean; // 是否可排序
    @Prop([Array, Number]) sortIndex?: any; // 开启排序的列序号数组或序号
    @Prop(Object) defaultSort?: object;
