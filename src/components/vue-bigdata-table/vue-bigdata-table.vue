      ...
      <div class="vue-bigdata-table-wrapper" ref="outWrapper">
        <div :class="['vue-bigdata-table-header-wrapper', fixed ? 'header-wrapper-fixed' : '']" :style="headerStyle">
          ...
          <table v-if="fixedCol >= 0"
                 :class="['vue-bigdata-table-fixed-header', showFixedBoxShadow ? 'box-shadow' : '']" cellspacing="0"
            ...
            <tr
              ...
              <th v-if="i <= fixedCol" v-for="(col, i) in columnsHandled" :data-index="i" :key="`table-title-${i}`"
          ...
          <table ref="headerTable" style="position: absolute;left: 0;top: 0;" cellspacing="0" cellpadding="0" border="0"
                 width="100%">
            <colgroup>
